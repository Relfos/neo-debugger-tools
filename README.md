...
The debugger will read the `.abi.json` file generated by the compiler to know what methods your smart contract support, and what inputs they need.
In order to insert arrays into the inputs separate each element with commas. For byte arrays, you can also just insert a single hexadecimal string (eg: FF03DA22)
